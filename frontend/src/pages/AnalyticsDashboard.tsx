...
const AnalyticsDashboard: React.FC = () => {
  ...
  const fetchOverview = async () => {
    ...
    try {
      ...
      else setFetchError('Failed to load analytics overview. Please refresh.');
      if (dashRes.status === 'fulfilled') setDashboard(dashRes.value.data);
    } catch (err: any) {
      setFetchError(err?.response?.data?.message || 'Failed to load analytics. Please refresh.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchStudentData = useCallback(async () => {
    ...
    try {
      ...
      params.append('limit', '500'); // Get students
      const res = await apiService.get(`/bitflow-owner/analytics/student-progress?${params.toString()}`);
      setStudentData(res.data);
    } catch (err: any) { setFetchError(err?.response?.data?.message || 'Failed to load student data.'); } finally { setTabLoading(false); }
  ...
  const fetchTeacherData = useCallback(async () => {
    ...
    try {
      ...
      if (stateFilter) params.append('state', stateFilter);
      if (cityFilter) params.append('city', cityFilter);
      params.append('limit', '500'); // Get teachers
      const res = await apiService.get(`/bitflow-owner/analytics/teacher-performance?${params.toString()}`);
      setTeacherData(res.data);
    } catch (err: any) { setFetchError(err?.response?.data?.message || 'Failed to load teacher data.'); } finally { setTabLoading(false); }
  }, [collegeFilter, stateFilter, cityFilter]);
  ...
  const fetchCourseData = useCallback(async () => {
    ...
    try {
      ...
      if (stateFilter) params.append('state', stateFilter);
      if (cityFilter) params.append('city', cityFilter);
      params.append('limit', '500'); // Get courses
      const res = await apiService.get(`/bitflow-owner/analytics/course-performance?${params.toString()}`);
      setCourseData(res.data);
    } catch (err: any) { setFetchError(err?.response?.data?.message || 'Failed to load course data.'); } finally { setTabLoading(false); }
  }, [collegeFilter, stateFilter, cityFilter]);
  ...
  const fetchCollegeComparison = useCallback(async () => {
    ...
    try {
      ...
      if (stateFilter) params.append('state', stateFilter);
      if (cityFilter) params.append('city', cityFilter);
      params.append('limit', '500'); // Get colleges
      const res = await apiService.get(`/bitflow-owner/analytics/college-comparison?${params.toString()}`);
      setCollegeData(res.data);
    } catch (err: any) { setFetchError(err?.response?.data?.message || 'Failed to load college data.'); } finally { setTabLoading(false); }
  }, [collegeFilter, stateFilter, cityFilter]);
    ...
    try {
      ...
      if (reportType === 'student-performance' && studentData?.students) {
        ...
        rows = dataSource.map((s: any) => [
          ...
          s.recentActivity?.lastLoginAt ? formatDate(s.recentActivity.lastLoginAt) : 'Never',
          s.recentActivity?.daysActive || 0,
        ]);
      } else if (reportType === 'teacher-performance' && teacherData?.teachers) {
        dataSource = teacherData.teachers;
        csvHeaders = ['Teacher Name', 'Email', 'College', 'State', 'City', 'Total Courses', 'Self-Paced Learning', 'Learning Unit', 'Active Courses', 'Total Students', 'Completed Students', 'Completion %', 'Avg Rating', 'Total Ratings', 'Last Active'];
        rows = dataSource.map((t: any) => [
          t.teacherName, t.email, t.collegeName, t.state || 'N/A', t.city || 'N/A',
          t.totalCourses, t.selfPacedCourses || 0, t.learningUnitCourses || 0, t.activeCourses,
          t.totalStudents, t.completedStudents, t.studentCompletionRate,
          t.avgRating || 'N/A', t.totalRatings || 0,
          t.lastActive ? formatDate(t.lastActive) : 'Never',
        ]);
      ...
      <div className="bo-page" style={{ background: 'linear-gradient(to bottom, #f8f9fc, #ffffff)' }}>
        {fetchError && (
          <div style={{
            ...
            padding: '12px 18px', marginBottom: 16, color: '#c53030',
            display: 'flex', alignItems: 'center', justifyContent: 'space-between'
          }}>
            <span>⚠️ {fetchError}</span>
            <button onClick={() => setFetchError('')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: '#c53030' }}>✕</button>
          </div>
        )}
        <div style={{
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              ...
              {analytics?.dailyActiveUsers && analytics.dailyActiveUsers.length > 0 && (
                <div style={{ background: 'white', padding: 28, borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', gridColumn: '1 / -1', marginBottom: 4 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, #667eea, #764ba2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
                      <TrendingUp size={22} />
                    </div>
                    <h3 style={{ fontSize: 17, fontWeight: 700, margin: 0, color: '#1f2937' }}>Daily Active Users (Last 30 Days)</h3>
                  </div>
                  <ResponsiveContainer width="100%" height={220}>
                    <LineChart data={analytics.dailyActiveUsers} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                      <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={d => d.slice(5)} />
                      <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
                      <Tooltip formatter={(v: number | undefined) => [v ?? 0, 'Active Users']} labelFormatter={d => `Date: ${d}`} />
                      <Line type="monotone" dataKey="count" stroke="#667eea" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
            ...
            {tabLoading ? (
              ...
              <>
                <div className="bo-stats-grid" style={{ marginBottom: 24 }}>
                  ...
                  <div className="bo-stat-card" style={{ position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ fontSize: 28, fontWeight: 800, color: '#1f2937', lineHeight: 1 }}>{teacherData.summary.totalCourses}</div>
                    <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4, marginBottom: 10, fontWeight: 500 }}>Total Courses</div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: '#667eea15', color: '#667eea', fontWeight: 600, border: '1px solid #667eea30' }}>
                        Self-Paced Learning: {teacherData.summary.selfPacedCourses ?? 0}
                      </span>
                      <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: '#8B5CF615', color: '#8B5CF6', fontWeight: 600, border: '1px solid #8B5CF630' }}>
                        Learning Unit: {teacherData.summary.learningUnitCourses ?? 0}
                      </span>
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                  <div style={{ display: 'flex', gap: 16 }}>
                    ...
                    <div style={{ padding: '8px 16px', background: 'var(--bo-success-light)', borderRadius: 8 }}>
                      <span style={{ fontSize: 12, color: 'var(--bo-text-muted)' }}>Avg MCQ Accuracy</span>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                        ...
                        <div style={{ display: 'flex', gap: 24 }}>
                          <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 11, color: 'var(--bo-text-muted)', marginBottom: 2 }}>Course Completion</div>
                            <div style={{ fontSize: 20, fontWeight: 700, color: getPerformanceColor(c.completionRate) }}>{c.completionRate}%</div>
                          </div>
                          <div style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 11, color: 'var(--bo-text-muted)', marginBottom: 2 }}>MCQ Accuracy</div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 16 }}>
                        <div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 11 }}>
                            <span style={{ color: 'var(--bo-text-muted)' }}>Course Completion Ratio</span>
                            <span style={{ fontWeight: 600, color: getPerformanceColor(c.completionRate) }}>{c.completedEnrollments}/{c.totalEnrollments}</span>
                          </div>
                          <ProgressBar value={c.completionRate} color={getPerformanceColor(c.completionRate)} height={6} />
                        </div>
                        <div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 11 }}>
                            <span style={{ color: 'var(--bo-text-muted)' }}>MCQ Accuracy</span>
                            <span style={{ fontWeight: 600, color: getPerformanceColor(c.avgAccuracy) }}>{c.avgAccuracy}%</span>
                          </div>
                        ...
                        <div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 11 }}>
                            ...
                            <span style={{ fontWeight: 600, color: getPerformanceColor(c.engagementScore) }}>{c.engagementScore}/100</span>
                          </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                  ...
                  <div style={{
                    ...
                    <div>
                      <div style={{ fontSize: 15, fontWeight: 600, color: '#1f2937', marginBottom: 4 }}>
                        {selectedTeacher.lastActive ? formatDate(selectedTeacher.lastActive) : 'Never'}
                      </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>Self-Paced</div>
                        <div style={{ fontSize: 20, fontWeight: 700, color: '#667eea' }}>{selectedTeacher.selfPacedCourses || 0}</div>
                      </div>
                      <div>
                        <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>Learning Unit</div>
                        <div style={{ fontSize: 20, fontWeight: 700, color: '#764ba2' }}>{selectedTeacher.learningUnitCourses || 0}</div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div style={{ padding: '16px', background: 'white', borderRadius: 10, border: '1px solid #e9ecef' }}>
                        ...
                        <div style={{ fontSize: 16, fontWeight: 700, color: '#212529' }}>
                          {selectedStudent.recentActivity.lastLoginAt
                            ? formatDate(selectedStudent.recentActivity.lastLoginAt)
                            : 'Never'}
                        </div>
                      </div>
                      <div style={{ padding: '16px', background: 'white', borderRadius: 10, border: '1px solid #e9ecef' }}>
                        ...
                        <div style={{ fontSize: 16, fontWeight: 700, color: '#212529' }}>
                          {selectedStudent.recentActivity.lastActivityAt
                            ? formatDate(selectedStudent.recentActivity.lastActivityAt)
                            : 'Never'}
                        </div>
                      </div>
