...
import {
  ...
  BarChart3, Upload, List, X, Check, Eye, Filter, FileQuestion, Image, FileText as FileTextIcon, ArrowLeft,
  ShieldCheck, ShieldOff, GraduationCap, Save
} from 'lucide-react';
...
const McqManagement: React.FC = () => {
  ...
  // View panel — inline competency editing
  const [viewCompetencyEdit, setViewCompetencyEdit] = useState(false);
  const [viewAllCompetencies, setViewAllCompetencies] = useState<Array<{
    id: string; code: string; title: string; description: string; subject: string; domain?: string;
  }>>([]);
  const [viewSelectedIds, setViewSelectedIds] = useState<string[]>([]);
  const [viewCompetencyLoading, setViewCompetencyLoading] = useState(false);
  const [viewCompetencySaving, setViewCompetencySaving] = useState(false);

  useEffect(() => {
    setViewCompetencyEdit(false);
    setViewAllCompetencies([]);
    setViewSelectedIds([]);
  }, [(viewingMcq as Mcq | null)?.id]);

  const openCompetencyEdit = async (mcq: Mcq) => {
    setViewSelectedIds(mcq.competencyIds || []);
    setViewCompetencyEdit(true);
    if (viewAllCompetencies.length === 0) {
      setViewCompetencyLoading(true);
      try {
        const res = await competencyService.getAll({ subject: mcq.subject, limit: 500 });
        setViewAllCompetencies(res.data.map(c => ({
          id: c.id, code: c.code, title: c.title,
          description: c.description, subject: c.subject, domain: c.domain as string | undefined,
        })));
      } catch { /* silent */ }
  ...
  const saveViewCompetencies = async () => {
    if (!viewingMcq) return;
    setViewCompetencySaving(true);
    try {
      const updated = await mcqService.update(viewingMcq.id, { competencyIds: viewSelectedIds });
      setViewingMcq({ ...viewingMcq, competencyIds: viewSelectedIds });
      setMcqs(prev => prev.map(m => m.id === updated.id ? updated : m));
      setViewCompetencyEdit(false);
    } catch { /* silent */ }
    setViewCompetencySaving(false);
  };
  ...
  }, [activeTab, page, filterSubject, filterDifficulty, filterVerified]);
  ...
  const handleVerify = async (id: string, approve: boolean) => {
    try {
      ...
      setListSuccess(approve ? 'MCQ verified and published!' : 'MCQ unverified.');
      setTimeout(() => setListSuccess(null), 3000);
      loadMcqs();
    } catch (err: any) {
      setListError(err?.response?.data?.message || 'Verify failed');
  ...
  const handleBulkVerify = async (idsToVerify?: string[]) => {
    ...
    try {
      ...
      setListSuccess(`Verified ${res.verified} MCQ${res.verified !== 1 ? 's' : ''}${res.alreadyVerified > 0 ? ` (${res.alreadyVerified} were already verified)` : ''}.`);
      setTimeout(() => setListSuccess(null), 5000);
      setSelectedIds(new Set());
      loadMcqs();
    } catch (err: any) {
      setListError(err?.response?.data?.message || 'Bulk verify failed');
      setTimeout(() => setListError(null), 4000);
    } finally {
  ...
  const handleBulkUnverify = async (idsToUnverify?: string[]) => {
    ...
    try {
      ...
      setListSuccess(`Unverified ${res.unverified} MCQ${res.unverified !== 1 ? 's' : ''}.`);
      setTimeout(() => setListSuccess(null), 5000);
      setSelectedIds(new Set());
      loadMcqs();
    } catch (err: any) {
      setListError(err?.response?.data?.message || 'Bulk unverify failed');
      setTimeout(() => setListError(null), 4000);
    } finally {
      setBulkVerifying(false);
    }
      ...
      {viewingMcq && (
        <div style={{
          ...
          <div className="bo-card" style={{ maxWidth: 700, width: '100%', maxHeight: '80vh', overflow: 'auto', padding: 24 }}
            ...
            {/* Competency Section */}
            <div style={{ marginTop: 16, padding: 16, background: 'var(--bo-bg)', borderRadius: 10, border: '1px solid var(--bo-border)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: viewCompetencyEdit ? 12 : 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: 'var(--bo-text)' }}>
                  <GraduationCap size={14} style={{ color: '#6366F1' }} />
                  Competencies
                  <span style={{ fontWeight: 400, color: 'var(--bo-text-muted)' }}>
                    ({(viewCompetencyEdit ? viewSelectedIds : viewingMcq.competencyIds)?.length || 0})
                  </span>
                </div>
                {!viewCompetencyEdit ? (
                  <button className="bo-btn bo-btn-outline" style={{ padding: '3px 10px', fontSize: 12 }}
                    onClick={() => openCompetencyEdit(viewingMcq)}>
                    <Edit2 size={12} /> Edit
                  </button>
                ) : (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button className="bo-btn bo-btn-outline" style={{ padding: '3px 10px', fontSize: 12 }}
                      disabled={viewCompetencySaving}
                      onClick={() => setViewCompetencyEdit(false)}>
                      <X size={12} /> Cancel
                    </button>
                    <button className="bo-btn bo-btn-primary" style={{ padding: '3px 10px', fontSize: 12 }}
                      disabled={viewCompetencySaving}
                      onClick={saveViewCompetencies}>
                      <Save size={12} /> {viewCompetencySaving ? 'Saving…' : 'Save'}
                    </button>
                  </div>
                )}
              </div>
              ...
              {!viewCompetencyEdit && (
                viewingMcq.competencyIds?.length > 0 ? (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {viewingMcq.competencyIds.map(id => {
                      const comp = viewAllCompetencies.find(c => c.id === id);
                      return (
                        <span key={id} style={{
                  ...
                  <div style={{ fontSize: 12, color: 'var(--bo-text-muted)', fontStyle: 'italic' }}>
                    No competencies mapped — click Edit to add
                  </div>
                )
              )}
              ...
              {viewCompetencyEdit && (
                viewCompetencyLoading ? (
                  <div style={{ padding: '16px 0', textAlign: 'center', fontSize: 13, color: 'var(--bo-text-muted)' }}>
                    Loading competencies…
                  </div>
                ) : (
                  <CompetencySearch
                    competencies={viewAllCompetencies}
                    selectedIds={viewSelectedIds}
                    onChange={setViewSelectedIds}
                    label="Map Competencies (CBME Repository)"
                    maxHeight="260px"
                  />
                )
              )}
        ...
        <>
          <div className="bo-card" style={{ padding: 16, marginBottom: 16 }}>
            <form onSubmit={handleSearchSubmit} style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <select value={filterVerified} onChange={e => { setFilterVerified(e.target.value); setPage(1); setSelectedIds(new Set()); }}
          ...
          {/* Status messages */}
          {listSuccess && <div style={{ padding: '10px 14px', borderRadius: 8, background: '#F0FDF4', border: '1px solid #BBF7D0', color: '#16A34A', fontSize: 13, marginBottom: 12 }}>{listSuccess}</div>}
          {listError && <div style={{ padding: '10px 14px', borderRadius: 8, background: '#FEF2F2', border: '1px solid #FECACA', color: '#DC2626', fontSize: 13, marginBottom: 12 }}>{listError}</div>}

          {/* Bulk action bar */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, minHeight: 36, flexWrap: 'wrap' }}>
            {selectedIds.size > 0 && (
              <>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
              ...
              {selectedIds.size > 0 && (
                <button className="bo-btn bo-btn-outline" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, borderColor: '#F59E0B', color: '#92400E' }}
                  disabled={bulkVerifying} onClick={() => {
                    const unselected = mcqs.filter(m => !selectedIds.has(m.id)).map(m => m.id);
                  ...
                  <thead>
                    <tr style={{ background: 'var(--bo-bg)', borderBottom: '1px solid var(--bo-border)' }}>
                      <th style={{ padding: '10px 14px', width: 36 }}>
                        <input type="checkbox" checked={selectedIds.size === mcqs.length && mcqs.length > 0}
                    ...
                    {mcqs.map(mcq => (
                      <tr key={mcq.id} style={{ borderBottom: '1px solid var(--bo-border)', cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '12px 14px' }} onClick={e => e.stopPropagation()}>
                          <input type="checkbox" checked={selectedIds.has(mcq.id)}
                            onChange={() => setSelectedIds(prev => {
                              const next = new Set(prev);
                              next.has(mcq.id) ? next.delete(mcq.id) : next.add(mcq.id);
                              return next;
                            })} />
                        </td>
                          ...
                          <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }} onClick={e => e.stopPropagation()}>
                            ...
                            {!mcq.isVerified ? (
                              ...
                              <button title="Unverify" className="bo-btn bo-btn-outline"
                                ...
                                {actionLoading === mcq.id ? '…' : <ShieldOff size={14} />}
                              </button>
                            )}
      ...
      {activeTab === 'create' && (
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          ...
          <form onSubmit={handleFormSubmit}>
                  ...
                  <div style={{ position: 'relative', display: 'inline-block', borderRadius: 8, overflow: 'hidden', border: '2px solid var(--bo-primary)', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
                    <img
                      src={formData.questionImage.startsWith('/uploads/')
                        ? `${API_BASE_URL.replace(/\/api$/, '')}${formData.questionImage}`
                        : formData.questionImage}
                      alt="MCQ Image"
            ...
            <div className="bo-card" style={{ padding: 20, marginBottom: 16 }}>
              <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Explanation</h3>
              <div>
