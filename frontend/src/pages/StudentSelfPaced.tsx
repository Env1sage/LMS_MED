...
const StudentSelfPaced: React.FC = () => {
  ...
  const handleResourceView = async (resource: SelfPacedResource) => {
    ...
    try {
      await apiService.post(`/self-paced/${resource.id}/access`, {});
    } catch (error) {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 24 }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 16, fontSize: 14, color: 'var(--bo-text-muted)', marginBottom: 12 }}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Clock size={14} /> {formatDate(selectedResource.createdAt)}
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {filteredResources.map((resource) => (
            ...
            >
              <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
                {resource.thumbnailUrl && getImageUrl(resource.thumbnailUrl) ? (
                  <div style={{
                    width: 48, height: 64, borderRadius: 'var(--bo-radius)', overflow: 'hidden',
                    border: '1px solid var(--bo-border)', flexShrink: 0, background: '#1a1a2e',
                  }}>
                    <img src={getImageUrl(resource.thumbnailUrl)} alt=""
                      style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                      onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                  </div>
                ) : (
                  <BookCover title={resource.title} type={resource.resourceType} subtitle={resource.subject} width={48} height={64} />
                )}
