    ...
    <PublisherLayout>
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          {profile?.logoUrl ? (
            <img src={profile.logoUrl} alt="" style={{ width: 52, height: 52, borderRadius: 12, objectFit: 'contain', border: '2px solid var(--bo-border)', background: '#f8faff' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          ) : (
            <div style={{ width: 52, height: 52, borderRadius: 12, background: 'linear-gradient(135deg,#8b5cf6,#a78bfa)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 22 }}>{(user?.fullName || profile?.companyName || 'P')[0].toUpperCase()}</div>
          )}
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--bo-text)' }}>Welcome back, {user?.fullName || profile?.companyName}</h1>
          ...
          {profile && (
            <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid var(--bo-border)' }}>
              ...
              <div style={{ fontSize: 13, display: 'grid', gap: 6 }}>
                ...
                <div><span style={{ color: 'var(--bo-text-muted)' }}>Since:</span> {formatDate(profile.createdAt)}</div>
                <div>
