...
const HodOnlineMeetings: React.FC = () => {
  ...
  const [detailMeeting, setDetailMeeting] = useState<OnlineMeeting | null>(null);
  ...
  const openEdit = (m: OnlineMeeting) => {
    setForm({
      title: m.title, description: m.description, speaker: m.speaker,
      speakerBio: m.speakerBio || '', type: m.type, date: m.date.slice(0, 10),
      startTime: m.startTime, endTime: m.endTime, venue: m.venue || '',
      meetingLink: m.meetingLink || '', capacity: m.capacity, department: m.department || '',
    });
    setEditId(m.id);
    setShowForm(true);
  };
  ...
  const toggleExpand = async (id: string) => {
    if (expanded === id) { setExpanded(null); setDetailMeeting(null); return; }
    try {
  ...
  const saveAttendance = async (meetingId: string) => {
    try {
      await apiService.post(`/guest-lectures/${meetingId}/attendance`, {
        studentIds: Array.from(attendanceIds),
      });
      const res = await apiService.get(`/guest-lectures/${meetingId}`);
      setDetailMeeting(res.data);
      const attended = new Set<string>();
      (res.data.registrations || []).forEach((r: any) => { if (r.attended) attended.add(r.student.id); });
      setAttendanceIds(attended);
  ...
  const stats = {
    ...
    totalRegistrations: meetings.reduce((s, m) => s + m.registrationCount, 0),
  };
    ...
    <HodLayout>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--bo-text)' }}>Online Meetings</h1>
            <p style={{ color: 'var(--bo-text-muted)', fontSize: 14 }}>Schedule and manage guest lectures, workshops & seminars for your department</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 24 }}>
          {[
            { label: 'Total Meetings', value: stats.total, color: ACCENT },
        ...
        {loading ? (
          ...
          <div style={{
            ...
            <p>No meetings scheduled yet. Schedule your first one!</p>
          </div>
        ) : (
            ...
            {filtered.map(m => (
              <div key={m.id} style={{
                ...
                <div style={{ padding: 20 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                        <span style={{
                          ...
                          background: `${statusColors[m.status]}15`, color: statusColors[m.status],
                        }}>{m.status}</span>
                        <span style={{
                      ...
                      <h3 style={{ fontSize: 17, fontWeight: 600, color: 'var(--bo-text)', margin: '4px 0' }}>{m.title}</h3>
                      <p style={{ fontSize: 13, color: 'var(--bo-text-muted)', margin: 0 }}>Speaker: <strong>{m.speaker}</strong></p>
                    </div>
                    <div style={{ display: 'flex', gap: 6 }}>
                      {m.status === 'UPCOMING' && (
                        <>
                          <button onClick={() => handleStatusChange(m.id, 'ONGOING')} title="Start" style={{
                      ...
                      {m.status === 'ONGOING' && (
                        <button onClick={() => handleStatusChange(m.id, 'COMPLETED')} style={{
                  ...
                  <div style={{ display: 'flex', gap: 20, marginTop: 12, flexWrap: 'wrap' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--bo-text-secondary)' }}>
                      <Calendar size={14} /> {formatDate(m.date)}
                    </span>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--bo-text-secondary)' }}>
                    ...
                    {m.meetingLink && (
                      <a href={m.meetingLink} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#3B82F6', textDecoration: 'none' }}>
                        <Video size={14} /> Join Online
                      </a>
                    )}
                    <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'var(--bo-text-secondary)' }}>
                      <Users size={14} /> {m.registrationCount}/{m.capacity}
                    </span>
                  </div>
                </div>
                ...
                <div style={{ borderTop: '1px solid var(--bo-border)' }}>
                  ...
                  {expanded === m.id && detailMeeting && (
                    <div style={{ padding: 20, background: '#FAFAFA' }}>
                      {(!detailMeeting.registrations || detailMeeting.registrations.length === 0) ? (
                        ...
                        <>
                          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                            ...
                            <tbody>
                              {detailMeeting.registrations.map(r => (
                          ...
                          <div style={{ marginTop: 12, textAlign: 'right' }}>
                            <button onClick={() => saveAttendance(m.id)} style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <h2 style={{ fontSize: 18, fontWeight: 700 }}>{editId ? 'Edit Meeting' : 'Schedule Online Meeting'}</h2>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div>
                    <label style={labelStyle}>Speaker / Presenter *</label>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 20 }}>
                ...
                }}>{saving ? 'Saving...' : editId ? 'Update' : 'Schedule'}</button>
              </div>
            </div>
          </div>
        )}
...
export default HodOnlineMeetings;
