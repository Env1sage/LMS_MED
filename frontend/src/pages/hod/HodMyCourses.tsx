  ...
  return (
    <HodLayout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>My Courses</h1>
      ...
      {filtered.length === 0 ? (
        <div className="bo-card" style={{ padding: 60, textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--bo-text-muted)', fontSize: 14 }}>Create a course to get started</p>
        </div>
      ) : (
            ...
            <div key={c.id} className="bo-card" style={{ padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginLeft: 16 }}>
                  <button onClick={() => navigate(`/hod/courses/${c.id}`)} title="View" style={{ padding: 8, border: '1px solid var(--bo-border)', borderRadius: 8, background: '#fff', cursor: 'pointer', color: 'var(--bo-text-secondary)' }}>
                    <Eye size={16} />
                  </button>
                  {c.status === 'DRAFT' && (
                    <>
                      <button onClick={() => navigate(`/hod/edit-course/${c.id}`)} title="Edit" style={{ padding: 8, border: '1px solid var(--bo-border)', borderRadius: 8, background: '#fff', cursor: 'pointer', color: '#F59E0B' }}>
                        <Edit2 size={16} />
                      </button>
                  ...
                  {c.status === 'PUBLISHED' && (
                    <>
                      <button onClick={() => navigate(`/hod/assign-course/${c.id}`)} title="Assign to Students" style={{ padding: 8, border: '1px solid var(--bo-border)', borderRadius: 8, background: '#fff', cursor: 'pointer', color: '#3B82F6' }}>
                        <Users size={16} />
                      </button>
                      <button onClick={() => navigate(`/hod/courses/${c.id}/analytics`)} title="Analytics" style={{ padding: 8, border: '1px solid var(--bo-border)', borderRadius: 8, background: '#fff', cursor: 'pointer', color: ACCENT }}>
                        <BarChart3 size={16} />
                      </button>
                    </>
                  )}
