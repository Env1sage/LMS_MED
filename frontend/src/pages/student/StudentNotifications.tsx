...
import {
  ArrowLeft, Bell, CheckCircle, Clock, AlertTriangle, BookOpen,
  ClipboardList, Megaphone, Check, CheckCheck
} from 'lucide-react';
          ...
          {filtered.map((notif) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 13, color: 'var(--bo-text-muted)', margin: 0, marginBottom: 6, lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden' }}>
                  {notif.message}
                </p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: 'var(--bo-text-muted)', flexWrap: 'wrap' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Clock size={12} /> {formatTimeAgo(notif.createdAt)}
                  </span>
                  {notif.senderName && <span>• {notif.senderName}</span>}
                  <span style={{ padding: '1px 8px', borderRadius: 10, background: 'var(--bo-border-light)', fontSize: 11, fontWeight: 500, textTransform: 'capitalize' }}>
                    {notif.category.toLowerCase()}
                  </span>
                  <span style={{ color: 'var(--bo-accent)', fontSize: 11 }}>Click to view details</span>
                </div>
              </div>
                ...
                {notif.actionUrl && (
                  <button
                    ...
                    onMouseLeave={(e) => {
                      ...
                      e.currentTarget.style.color = 'var(--bo-accent)';
                    }}
                  >
                    Open
                  </button>
