          ...
          return (
            <div key={book.id} className="bo-card" style={{ padding: 20, display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
                {book.coverUrl && getAuthImageUrl(book.coverUrl) ? (
                  <div style={{ width: 80, height: 110, borderRadius: 6, overflow: 'hidden', flexShrink: 0, border: '1px solid var(--bo-border)', background: '#1a1a2e' }}>
                    <img src={getAuthImageUrl(book.coverUrl)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                  </div>
                ) : (
                  <BookCover title={book.title} subtitle={book.subject} type="BOOK" width={80} height={110} />
                )}
