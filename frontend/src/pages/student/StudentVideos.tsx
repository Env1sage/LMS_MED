        ...
        {filteredVideos.map(video => {
          ...
          return (
            <div key={video.id} className="bo-card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              onClick={() => navigate(`/student/library/${video.id}/view`)}
              >
                {video.thumbnailUrl && getAuthImageUrl(video.thumbnailUrl) ? (
                  <img src={getAuthImageUrl(video.thumbnailUrl)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', top: 0, left: 0 }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                ) : (
                  <BookCover title={video.title} subtitle={video.subject} type="VIDEO" width={320} height={180} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', borderRadius: 0 }} />
                )}
                <Play size={48} style={{ color: 'white', opacity: 0.9, position: 'relative', zIndex: 1, filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} />
                <div style={{
                  position: 'absolute',
                  bottom: 8,
                  right: 8,
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--bo-text-muted)', marginBottom: 16 }}>
                  {video.views?.toLocaleString() ?? 0} views • {formatDate(video.uploadedAt)}
                </div>
