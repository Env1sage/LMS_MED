...
import {
  ArrowLeft, BookOpen, TrendingUp, Award, Users, RefreshCw, BarChart3,
                      ...
                      <tbody>
                        {courseDetail.studentPerformance.map((sp, i) => (
                          <tr key={i} style={{ borderBottom: '1px solid var(--bo-border)', transition: 'background 0.15s' }}
                            ...
                            <td style={{ padding: '12px 14px', textAlign: 'center', fontSize: 12, color: 'var(--bo-text-muted)' }}>{sp.startedAt ? formatDate(sp.startedAt) : '—'}</td>
                            <td style={{ padding: '12px 14px', textAlign: 'center', fontSize: 12, color: 'var(--bo-text-muted)' }}>{sp.completedAt ? formatDate(sp.completedAt) : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                          ...
                          <tbody>
                            {test.attempts.slice(0, 10).map((a, ai) => (
                              <tr key={ai} style={{ borderBottom: '1px solid var(--bo-border)' }}>
                                ...
                                <td style={{ padding: '10px 14px', color: 'var(--bo-text-muted)', fontSize: 11 }}>{formatDate(a.submittedAt)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
