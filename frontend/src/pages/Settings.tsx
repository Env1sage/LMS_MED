...
import { Settings as SettingsIcon, Shield, Save, CheckCircle, XCircle, ToggleLeft, ToggleRight, User, Lock } from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'security' | 'features' | 'profile'>('security');
    ...
    <MainLayout>
      <div className="bo-page">
        ...
        <div className="bo-tabs" style={{ marginBottom: 20 }}>
          ...
          <button className={`bo-tab ${activeTab === 'profile' ? 'bo-tab-active' : ''}`} onClick={() => setActiveTab('profile')}>
        ...
        {activeTab === 'profile' && (
          <div className="bo-card" style={{ padding: 28 }}>
            <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 20 }}>Change Password</h3>
            {profileError && <div className="bo-alert bo-alert-error" style={{ marginBottom: 16 }}><XCircle size={16} />{profileError}</div>}
            {profileSuccess && <div className="bo-alert bo-alert-success" style={{ marginBottom: 16 }}><CheckCircle size={16} />{profileSuccess}</div>}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 400 }}>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>Current Password</label>
                <input
                  type="password"
                  className="bo-input"
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>New Password</label>
                <input
                  type="password"
                  className="bo-input"
              ...
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#374151' }}>Confirm New Password</label>
                <input
                  type="password"
                  className="bo-input"
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                ...
                >
                  {saving ? <><div className="bo-spinner" style={{ width: 16, height: 16, borderWidth: 2 }} /> Saving...</> : <><Lock size={16} /> Change Password</>}
                </button>
              </div>
            </div>
          </div>
