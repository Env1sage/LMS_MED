...
const CollegeAdminDashboard: React.FC = () => {
  ...
  const [collegeProfile, setCollegeProfile] = useState<{ maxStudents?: number | null; _count?: { users: number } } | null>(null);
  ...
  const loadDashboard = useCallback(async (silent = false) => {
    try {
      ...
      const [statsRes, deptRes, facultyRes, analyticsRes, pkgRes, profileRes] = await Promise.allSettled([
      ...
      if (profileRes.status === 'fulfilled') setCollegeProfile(profileRes.value?.data || null);
  ...
  const formatYear = (y: string) => {
    const m: Record<string, string> = { FIRST_YEAR: '1st Year', SECOND_YEAR: '2nd Year', THIRD_YEAR: '3rd Year', FOURTH_YEAR: '4th Year', FIFTH_YEAR: '5th Year', INTERNSHIP: 'Internship', YEAR_1: '1st Year', YEAR_2: '2nd Year', YEAR_3: '3rd Year', YEAR_4: '4th Year', YEAR_5: '5th Year', YEAR_3_PART1: 'Year 3 (Part 1)', YEAR_3_PART2: 'Year 3 (Part 2)', PART_1: 'Part 1', PART_2: 'Part 2' };
  ...
  const maxAccounts = collegeProfile?.maxStudents ?? null;
  const usedAccounts = collegeProfile?._count?.users ?? totalStudents;
  const remainingAccounts = maxAccounts !== null ? Math.max(0, maxAccounts - usedAccounts) : null;
  const usagePct = maxAccounts ? Math.min(100, (usedAccounts / maxAccounts) * 100) : 0;
  const usageColor = maxAccounts
  ...
  return (
    <CollegeLayout>
      ...
      <div className="bo-card" style={{ padding: '16px 20px', marginBottom: 20, borderLeft: `4px solid ${usageColor}` }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: `${usageColor}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: usageColor }}>
              <Users size={20} />
            </div>
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--bo-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Account Usage</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: usageColor, marginTop: 2 }}>
                {usedAccounts.toLocaleString()}
                {maxAccounts !== null && <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--bo-text-muted)' }}> / {maxAccounts.toLocaleString()} allocated</span>}
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 24 }}>
            {maxAccounts !== null && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: remainingAccounts === 0 ? '#EF4444' : '#10B981' }}>{remainingAccounts?.toLocaleString()}</div>
                <div style={{ fontSize: 11, color: 'var(--bo-text-muted)' }}>Remaining</div>
              </div>
            )}
            {maxAccounts !== null && (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: usageColor }}>{usagePct.toFixed(0)}%</div>
                <div style={{ fontSize: 11, color: 'var(--bo-text-muted)' }}>Used</div>
              </div>
            )}
            {maxAccounts === null && (
              <div style={{ fontSize: 13, color: 'var(--bo-text-muted)', alignSelf: 'center' }}>No account limit set</div>
            )}
          </div>
          {maxAccounts !== null && (
            <div style={{ flex: '1 1 200px', minWidth: 200 }}>
              <div style={{ height: 8, borderRadius: 4, background: '#E5E7EB', overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: 4, background: usageColor, width: `${usagePct}%`, transition: 'width 0.4s ease' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4, fontSize: 11, color: 'var(--bo-text-muted)' }}>
                <span>0</span><span>{maxAccounts.toLocaleString()}</span>
              </div>
            </div>
          )}
        </div>
