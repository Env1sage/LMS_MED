...
const AuditLogs: React.FC = () => {
  ...
  // Fetch publishers and colleges for filter dropdowns
  useEffect(() => {
    const fetchFilterOptions = async () => {
      try {
        const [pubRes, colRes] = await Promise.all([
          apiService.get('/bitflow-owner/publishers').catch(() => ({ data: [] })),
          apiService.get('/bitflow-owner/colleges').catch(() => ({ data: [] })),
        ]);
        const pubs = pubRes.data?.publishers || pubRes.data?.data || pubRes.data || [];
        setPublishers(Array.isArray(pubs) ? pubs : []);
        const cols = colRes.data?.colleges || colRes.data?.data || colRes.data || [];
        setCollegesList(Array.isArray(cols) ? cols : []);
      } catch (err) {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      // If a category is selected, send all actions in that category as a comma-separated string
      ...
      if (publisherFilter) params.publisherId = publisherFilter;
      if (collegeFilter) params.collegeId = collegeFilter;
  ...
  }, [page, actionCategory, roleFilter, publisherFilter, collegeFilter, dateFrom, dateTo, searchTerm]);
  ...
  const filtered = logs.filter(log => {
    ...
    // Publisher filter
    if (publisherFilter && log.publisherName?.toLowerCase() !== publishers.find(p => p.id === publisherFilter)?.name?.toLowerCase()) return false;
    // College filter
    if (collegeFilter && log.collegeName?.toLowerCase() !== collegesList.find(c => c.id === collegeFilter)?.name?.toLowerCase()) return false;
    if (!searchTerm) return true;
    const term = searchTerm.toLowerCase();
    return (log.action?.toLowerCase().includes(term)) ||
      (log.userEmail?.toLowerCase().includes(term)) ||
      ...
      <div className="bo-page">
        ...
        {showFilters && (
          <div style={{
            ...
            {/* Publisher Filter */}
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--bo-text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>Publisher</label>
              <select
                className="bo-filter-select"
                value={publisherFilter}
                onChange={e => { setPublisherFilter(e.target.value); setPage(1); }}
                style={{ width: '100%', padding: '8px 12px', fontSize: 13, borderRadius: 8 }}
              >
            ...
            {/* College Filter */}
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--bo-text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6, display: 'block' }}>College</label>
              <select
                className="bo-filter-select"
                value={collegeFilter}
        ...
        {activeFilterCount > 0 && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
            ...
            {publisherFilter && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px 4px 12px', borderRadius: 99, fontSize: 12, fontWeight: 500, background: '#FFF7ED', color: '#C2410C' }}>
                Publisher: {publishers.find(p => p.id === publisherFilter)?.name || 'Unknown'}
                <X size={12} style={{ cursor: 'pointer' }} onClick={() => { setPublisherFilter(''); setPage(1); }} />
              </span>
            )}
            {collegeFilter && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px 4px 12px', borderRadius: 99, fontSize: 12, fontWeight: 500, background: '#F0FDF4', color: '#166534' }}>
                College: {collegesList.find(c => c.id === collegeFilter)?.name || 'Unknown'}
                <X size={12} style={{ cursor: 'pointer' }} onClick={() => { setCollegeFilter(''); setPage(1); }} />
              </span>
            )}
            {dateFrom && (
