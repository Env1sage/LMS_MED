...
const FacultyStudents: React.FC = () => {
  ...
  const loadStudents = useCallback(async () => {
    try {
      setLoading(true);
      const res = await facultyAnalyticsService.getAllStudents(filter, yearFilter);
  ...
  const YEAR_LABELS: Record<string, string> = {
    ...
    FOURTH_YEAR: '4th Year', FIFTH_YEAR: '5th Year',
    INTERNSHIP: 'Internship', PART_1: 'Part 1', PART_2: 'Part 2',
  };

  const availableYears = useMemo(() => {
    const ys = Array.from(new Set(students.map(s => s.academicYear).filter(Boolean))) as string[];
    return ys.sort();
  }, [students]);

  const filtered = students
    .filter(s => !search || s.name.toLowerCase().includes(search.toLowerCase()) || s.email.toLowerCase().includes(search.toLowerCase()))
    .filter(s => yearFilter === 'all' || s.academicYear === yearFilter)
    .sort((a, b) => {
      let cmp = 0;
    ...
    <FacultyLayout>
      ...
      {availableYears.length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--bo-text-muted)', textTransform: 'uppercase', letterSpacing: 0.5 }}>Year:</span>
          {['all', ...availableYears].map(y => (
            <button key={y} onClick={() => setYearFilter(y)}
              style={{
                padding: '6px 14px', borderRadius: 20, border: yearFilter === y ? `1.5px solid ${ACCENT}` : '1.5px solid var(--bo-border)',
                background: yearFilter === y ? '#F5F3FF' : '#fff',
                color: yearFilter === y ? ACCENT : 'var(--bo-text-secondary)',
                fontWeight: 600, fontSize: 12, cursor: 'pointer', transition: 'all 0.15s',
              }}>
              {y === 'all' ? 'All Years' : (YEAR_LABELS[y] || y)}
            </button>
          ))}
        </div>
      )}
              ...
              <tbody>
                {filtered.map((s, idx) => (
                  <tr key={s.id} style={{ borderTop: '1px solid var(--bo-border)', transition: 'background 0.15s' }}
                    ...
                    <td style={{ padding: '14px 16px', fontSize: 13 }}>{YEAR_LABELS[s.academicYear] || s.academicYear || '—'}</td>
