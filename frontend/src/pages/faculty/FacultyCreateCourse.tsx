...
const FacultyCreateCourse: React.FC = () => {
  ...
  const addCustomContent = async () => {
    if (!customContent.title.trim() || !customContent.fileUrl) {
      setModalError('Please fill in title and upload a file');
      return;
    }
    if (customContent.title.trim().length < 2) {
      setModalError('Title must be at least 2 characters');
      return;
    }
    ...
    try {
      ...
      const errorMsg = Array.isArray(msg) ? msg.join(', ') : (msg || err.message || 'Failed to create learning unit');
        ...
        <div className="bo-card" style={{ padding: 24, marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button type="button" className="bo-btn bo-btn-outline" onClick={() => { setShowUploadModal(true); setModalError(''); }} style={{ borderColor: '#10B981', color: '#10B981' }}>
      ...
      {showUploadModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowUploadModal(false)}>
          <div style={{ background: '#fff', borderRadius: 12, width: '90%', maxWidth: 600, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div className="bo-card" style={{ padding: 16, border: '2px dashed var(--bo-border)', background: 'var(--bo-bg)' }}>
                  {customContent.fileUrl ? (
                    ...
                    <FileUploadButton
                      fileType={customContent.type === 'VIDEO' ? 'video' : customContent.type === 'PPT' ? 'ppt' : customContent.type === 'NOTES' || customContent.type === 'DOCUMENT' ? 'document' : 'book'}
            ...
            <div style={{ padding: '16px 24px', borderTop: '1px solid var(--bo-border)' }}>
              {modalError && (
