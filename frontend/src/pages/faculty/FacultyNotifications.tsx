...
import { ArrowLeft, Bell, Check, Send, X, AlertCircle } from 'lucide-react';
          ...
          {filtered.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filtered.map(n => {
                ...
                return (
                  <div key={n.id} className="bo-card" style={{ padding: 0, overflow: 'hidden', borderLeft: `4px solid ${n.isRead ? 'var(--bo-border)' : tc.color}`, opacity: n.isRead ? 0.8 : 1, cursor: 'pointer' }}
                    onClick={() => { if (!n.isRead) markAsRead(n.id); setSelectedNotif(n); }}>
                    <div style={{ padding: '16px 20px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ fontSize: 13, color: 'var(--bo-text-secondary)', margin: '0 0 8px', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden' }}>{n.message}</p>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          <span style={{ fontSize: 11, color: tc.color, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                            {tc.icon} {n.type.replace('_', ' ')}
                          </span>
                          {!n.isRead && (
                            <button onClick={(e) => { e.stopPropagation(); markAsRead(n.id); }} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 6, border: '1px solid var(--bo-border)', background: '#fff', cursor: 'pointer', fontSize: 12, color: ACCENT, fontWeight: 600 }}>
          ...
          {sentNotifications.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {sentNotifications.map((n: any) => {
                ...
                const pc = priorityConfig[n.priority] || priorityConfig.NORMAL;
                return (
                  <div key={n.id} className="bo-card" style={{ padding: 0, overflow: 'hidden', borderLeft: `4px solid ${tc.color}`, cursor: 'pointer' }}
