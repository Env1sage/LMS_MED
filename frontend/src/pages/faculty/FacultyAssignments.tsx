...
import {
  ArrowLeft, ClipboardList, Plus, Users, CheckCircle, Clock,
  X, Search, Award, Trash2, BookOpen
} from 'lucide-react';
...
const FacultyAssignments: React.FC = () => {
  ...
  // MCQ selection state
  const [availableMcqs, setAvailableMcqs] = useState<AvailableMCQ[]>([]);
  const [selectedMcqIds, setSelectedMcqIds] = useState<Set<string>>(new Set());
  const [mcqSearch, setMcqSearch] = useState('');
  const [mcqLoading, setMcqLoading] = useState(false);
  const [mcqFilter, setMcqFilter] = useState<'all' | 'publisher' | 'own'>('all');
  const [formMarksPerQ, setFormMarksPerQ] = useState(1);
  const [formNegativeMarking, setFormNegativeMarking] = useState(false);
  const [formNegativeValue, setFormNegativeValue] = useState(0.25);
  ...
  const loadStudentsForCourse = async (_courseId: string) => {
    try {
  ...
  const loadAvailableMcqs = async (search?: string) => {
    try {
      setMcqLoading(true);
      const params = new URLSearchParams();
      if (search) params.set('search', search);
      const res = await apiService.get(`/faculty/assignments/available-mcqs?${params.toString()}`);
      setAvailableMcqs(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
  ...
  const filteredMcqs = availableMcqs.filter(m => {
    if (mcqFilter === 'publisher') return m.source === 'PUBLISHER';
    if (mcqFilter === 'own') return m.source === 'FACULTY';
    return true;
  });
  ...
  const handleCreate = async () => {
    try {
      ...
      if (selectedMcqIds.size === 0) { setError('Please select at least 1 MCQ for the assignment'); return; }
      setError('');
      const mcqIdsArray = Array.from(selectedMcqIds);
      await facultyAssignmentService.create({
        ...form,
        totalMarks: selectedMcqIds.size * formMarksPerQ,
        passingMarks: Number(form.passingMarks),
        studentIds: selectedStudents,
        mcqIds: mcqIdsArray,
        durationMinutes: formDuration,
        marksPerQuestion: formMarksPerQ,
        negativeMarking: formNegativeMarking,
        negativeMarkValue: formNegativeMarking ? formNegativeValue : 0,
        shuffleQuestions: formShuffle,
      });
    ...
    <FacultyLayout>
      ...
      {view === 'list' && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            ...
            <button onClick={() => { setView('create'); loadResources(); loadAvailableMcqs(); loadStudentsForCourse(''); setForm({ courseId: '', title: '', description: '', totalMarks: 100, passingMarks: 40, dueDate: '', startDate: '', selfPacedResourceId: '' }); setSelectedStudents([]); setSelectedMcqIds(new Set()); }}
          ...
          {filtered.length === 0 ? (
            <div className="bo-card" style={{ padding: 60, textAlign: 'center' }}>
              ...
              <button onClick={() => { setView('create'); loadResources(); loadAvailableMcqs(); }} className="bo-btn bo-btn-primary" style={{ background: ACCENT }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 20, fontSize: 12, color: 'var(--bo-text-muted)' }}>
                          ...
                          {a.dueDate && (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: isPastDue ? '#EF4444' : 'inherit' }}>
                              <Clock size={13} /> Due: {formatDate(a.dueDate)}
                            </span>
                          )}
                        </div>
      ...
      {view === 'create' && (
        <>
            ...
            <div>
              <div className="bo-card" style={{ padding: 24 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 16 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6, color: 'var(--bo-text-secondary)' }}>Marks/Question</label>
                    <input type="number" value={formMarksPerQ} onChange={e => setFormMarksPerQ(Number(e.target.value))} min={1}
                      style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid var(--bo-border)', fontSize: 14 }} />
                  </div>
                  <div>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: 13, fontWeight: 500, marginBottom: 6, color: 'var(--bo-text-secondary)' }}>Duration (min)</label>
                    <input type="number" value={formDuration} onChange={e => setFormDuration(Number(e.target.value))} min={0}
                      style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid var(--bo-border)', fontSize: 14 }} />
                  </div>
                </div>
                ...
                {selectedMcqIds.size > 0 && (
                  <div style={{ padding: '10px 14px', background: '#F5F3FF', borderRadius: 8, marginBottom: 16, fontSize: 13, color: ACCENT, fontWeight: 600 }}>
                    {selectedMcqIds.size} MCQs × {formMarksPerQ} marks = {selectedMcqIds.size * formMarksPerQ} total marks
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer' }}>
                    <input type="checkbox" checked={formShuffle} onChange={e => setFormShuffle(e.target.checked)} style={{ accentColor: ACCENT }} />
                    Shuffle Questions
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer' }}>
                    <input type="checkbox" checked={formNegativeMarking} onChange={e => setFormNegativeMarking(e.target.checked)} style={{ accentColor: ACCENT }} />
                    Negative Marking
                  </label>
                  {formNegativeMarking && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <span style={{ fontSize: 13 }}>Deduct:</span>
                      <input type="number" step="0.25" value={formNegativeValue} onChange={e => setFormNegativeValue(Number(e.target.value))}
                        style={{ width: 70, padding: '6px 10px', borderRadius: 8, border: '1px solid var(--bo-border)', fontSize: 13 }} min={0} />
                    </div>
                  )}
                </div>
                ...
                <button onClick={handleCreate} className="bo-btn bo-btn-primary" style={{ width: '100%', background: ACCENT, padding: '12px', fontWeight: 600, fontSize: 15 }}>
                  <ClipboardList size={16} /> Create Assignment ({selectedMcqIds.size} MCQs)
                </button>
              </div>
            </div>
          ...
          {/* MCQ Selection Section (full width below) */}
          <div className="bo-card" style={{ padding: 24, marginTop: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div>
                <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <BookOpen size={18} /> Select MCQs for Assignment
                </h3>
                <p style={{ fontSize: 12, color: 'var(--bo-text-muted)', margin: '4px 0 0' }}>
                  Choose from publisher MCQs and your own MCQ bank ({selectedMcqIds.size} selected)
                </p>
              </div>
              <div style={{ display: 'flex', gap: 6 }}>
                {(['all', 'publisher', 'own'] as const).map(f => (
                  <button key={f} onClick={() => setMcqFilter(f)} style={{
                    padding: '5px 14px', borderRadius: 16, fontSize: 12, fontWeight: 500, border: 'none', cursor: 'pointer',
                    background: mcqFilter === f ? ACCENT : '#f3f4f6', color: mcqFilter === f ? '#fff' : '#374151',
                  }}>
                    {f === 'all' ? 'All' : f === 'publisher' ? 'Publisher' : 'My MCQs'}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ position: 'relative', marginBottom: 12 }}>
              <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--bo-text-muted)' }} />
              <input placeholder="Search MCQs by question, subject, topic..." value={mcqSearch}
                onChange={e => { setMcqSearch(e.target.value); loadAvailableMcqs(e.target.value); }}
                style={{ width: '100%', padding: '8px 10px 8px 32px', borderRadius: 8, border: '1px solid var(--bo-border)', fontSize: 13 }} />
            </div>

            <div style={{ maxHeight: 350, overflow: 'auto', border: '1px solid #e5e7eb', borderRadius: 8 }}>
              {mcqLoading ? (
                <div style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>Loading MCQs...</div>
              ) : filteredMcqs.length === 0 ? (
                <div style={{ padding: 20, textAlign: 'center', color: '#9ca3af' }}>No MCQs available</div>
              ) : (
                filteredMcqs.map(m => {
                  const isSelected = selectedMcqIds.has(m.id);
                  return (
                    <div key={m.id} onClick={() => toggleMcq(m.id)} style={{
                      padding: '10px 14px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'flex-start', gap: 10,
                      cursor: 'pointer', background: isSelected ? '#f5f3ff' : '#fff', transition: 'background 0.15s',
                    }}>
                      <div style={{
                        width: 20, height: 20, borderRadius: 4, flexShrink: 0, marginTop: 2,
                        border: isSelected ? `2px solid ${ACCENT}` : '2px solid #d1d5db', background: isSelected ? ACCENT : '#fff',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                      }}>
                        {isSelected && <CheckCircle size={14} color="#fff" />}
                      </div>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontSize: 13, color: 'var(--bo-text-primary)', marginBottom: 4, lineHeight: 1.4, margin: 0 }}>
                          {m.question.length > 180 ? m.question.slice(0, 180) + '...' : m.question}
                        </p>
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                          {m.subject && <span style={{ fontSize: 10, padding: '1px 8px', borderRadius: 8, background: '#f3f4f6', color: '#6b7280' }}>{m.subject}</span>}
                          {m.topic && <span style={{ fontSize: 10, padding: '1px 8px', borderRadius: 8, background: '#f3f4f6', color: '#6b7280' }}>{m.topic}</span>}
                          {m.difficultyLevel && <span style={{ fontSize: 10, padding: '1px 8px', borderRadius: 8, background: '#fef3c7', color: '#92400e' }}>{m.difficultyLevel}</span>}
                          <span style={{ fontSize: 10, padding: '1px 8px', borderRadius: 8, background: m.source === 'PUBLISHER' ? '#eff6ff' : '#f5f3ff', color: m.source === 'PUBLISHER' ? '#2563eb' : ACCENT }}>
                            {m.source === 'PUBLISHER' ? 'Publisher' : 'My MCQ'}
                          </span>
                        </div>
                      </div>
                    </div>
