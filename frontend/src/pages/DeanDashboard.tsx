                ...
                {notifications.slice(0, 5).map(notif => (
                  <div key={notif.id} style={{ display: 'flex', gap: 10, padding: '8px 0', borderBottom: '1px solid var(--bo-border)', fontSize: 13, opacity: notif.isRead ? 0.6 : 1 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--bo-text-muted)' }}>{formatDate(notif.createdAt)}</div>
                    </div>
                  </div>
                ))}
                  ...
                  <tbody>
                    {faculty.map(f => (
                      <tr key={f.id}>
                        ...
                        <td style={{ fontSize: 13, color: 'var(--bo-text-muted)' }}>{f.lastLoginAt ? formatDate(f.lastLoginAt) : 'Never'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
