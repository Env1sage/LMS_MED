    ...
    <MainLayout>
      <div className="bo-page">
        ...
        <div className="bo-stats-grid">
          ...
          <div className="bo-stat-card" onClick={() => navigate('/analytics')}>
        ...
        <div className="bo-grid-2">
          ...
          <div className="bo-card">
            <div className="bo-card-header">
              <h3 className="bo-card-title">Recent Colleges — Account Usage</h3>
            ...
            <div className="bo-card-body" style={{ padding: 0 }}>
              {colleges.length > 0 ? (
                colleges.slice(0, 5).map((col: any) => {
                  const used = col.userCount ?? col.totalStudents ?? 0;
                  const max = col.maxStudents ?? null;
                  const pct = max ? Math.min(100, (used / max) * 100) : 0;
                  const color = max ? (pct >= 90 ? '#EF4444' : pct >= 70 ? '#F59E0B' : '#10B981') : '#6B7280';
                  return (
                    <div
                      key={col.id}
                      className="bo-list-item"
                      style={{ borderRadius: 0, border: 'none', borderBottom: '1px solid var(--bo-border-light)', flexDirection: 'column', alignItems: 'stretch', gap: 6, cursor: 'pointer' }}
                      onClick={() => navigate('/colleges')}
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                          <div style={{ fontWeight: 600, color: 'var(--bo-text-primary)', fontSize: '14px' }}>{col.name}</div>
                          <div style={{ fontSize: '12px', color: 'var(--bo-text-muted)', marginTop: 1 }}>
                            <span style={{ color, fontWeight: 600 }}>{used.toLocaleString()}</span>
                            {max !== null ? <span style={{ color: 'var(--bo-text-muted)' }}> / {max.toLocaleString()} accounts</span> : <span style={{ color: 'var(--bo-text-muted)' }}> accounts (no limit)</span>}
                            {max !== null && <span style={{ marginLeft: 8, color: color }}>{(max - used) > 0 ? `· ${(max - used).toLocaleString()} left` : '· At limit'}</span>}
                          </div>
                        </div>
                        <span className={`bo-badge ${col.status === 'ACTIVE' ? 'bo-badge-success' : 'bo-badge-warning'}`}>{col.status}</span>
                      </div>
                      {max !== null && (
          ...
          <div className="bo-card-body">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
              {[
                ...
                { icon: Shield, label: 'Audit Logs', sub: 'Security & activity', path: '/activity-logs' },
