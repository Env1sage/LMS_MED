...
interface LearningUnit {
  ...
  title: string;
  description: string;
  type: 'BOOK' | 'VIDEO' | 'INTERACTIVE' | 'MCQ';
...
const StudentCourseView: React.FC = () => {
  ...
  const handleStartLearning = async (step: LearningStep) => {
    const type = step.learning_units.type;

    // For MCQ type, find the specific test matching this unit's subject
    if (type === 'MCQ' || type === 'INTERACTIVE') {
      try {
        const testsRes = await apiService.get('/student-portal/tests');
        const tests = Array.isArray(testsRes.data) ? testsRes.data : [];
        const unitTitle = (step.learning_units.title || '').toLowerCase();
        let matchedTest = tests.find((t: any) => {
          const testSubject = (t.subject || '').toLowerCase();
          return testSubject && unitTitle.includes(testSubject);
        });
        if (!matchedTest) matchedTest = tests.find((t: any) => t.courseId === courseId);
        if (!matchedTest && course) {
          const courseTitle = course.title.toLowerCase();
          matchedTest = tests.find((t: any) => {
            const testSubject = (t.subject || '').toLowerCase();
            return courseTitle.includes(testSubject) || testSubject.includes(courseTitle.split(' ')[0]);
          });
        }
        if (matchedTest) { navigate(`/student/tests/${matchedTest.id}`); return; }
      } catch (err) {
        console.error('Error finding test for course:', err);
      }
      navigate('/student/assignments');
      return;
    }

    // For VIDEO: open in secure dialog and auto-submit progress immediately
    if (type === 'VIDEO') {
      setCurrentUnit(step.learning_units);
      setViewerOpen(true);
      setStartTime(Date.now());
      // Auto-submit completion immediately when video is opened
      try {
        ...
        console.error('Error auto-submitting video progress:', err);
      }
      return;
    }

    // For EPUB/BOOK/NOTES — navigate to proper content viewer and auto-submit progress
    try {
      await apiService.post('/progress/submit', {
        learning_flow_stepsId: step.id,
        completionPercent: 100,
      ...
      console.error('Error auto-submitting content progress:', err);
    }
    navigate(`/student/library/${step.learning_units.id}/view`, {
      state: { fromCourseId: courseId, stepId: step.id },
    });
  };

  const handleCloseViewer = async () => {
    // Progress was already submitted when content was opened — just close the dialog
    setViewerOpen(false);
    setCurrentUnit(null);
    setStartTime(0);
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4, userSelect: 'none' }} onContextMenu={(e) => e.preventDefault()}>
                ...
                >
                  <Box display="flex" alignItems="center" gap={1.5}>
                    {step.learning_units.thumbnailUrl && getAuthImageUrl(step.learning_units.thumbnailUrl) ? (
                      <Box
                        component="img"
                        src={getAuthImageUrl(step.learning_units.thumbnailUrl)}
                        alt=""
                        sx={{
                          width: 40, height: 54, borderRadius: 1, objectFit: 'cover',
                          border: '1px solid', borderColor: 'divider', flexShrink: 0,
                        }}
                        onError={(e: any) => { e.target.style.display = 'none'; }}
                      />
                    ) : (
                      <BookCover title={step.learning_units.title} type={step.learning_units.type} width={40} height={54} />
                    )}
          ...
          <Box sx={{ width: '100%', height: '100%', position: 'relative', bgcolor: '#f5f5f5' }}>
            ...
            {currentUnit?.secureAccessUrl.match(/\.(mp4|webm|ogg)$/i) && (
              <Box sx={{ width: '100%', height: '100%', bgcolor: 'black', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                ...
                >
                  <source src={`${getFullContentUrl(currentUnit.secureAccessUrl)}?token=${localStorage.getItem('accessToken') || ''}`} />
            ...
            {currentUnit && currentUnit.secureAccessUrl.match(/\.pdf$/i) && (
              <iframe
                src={`${getFullContentUrl(currentUnit.secureAccessUrl)}?token=${localStorage.getItem('accessToken') || ''}`}
                title={currentUnit.title}
                style={{ width: '100%', height: '100%', border: 'none' }}
                onError={() => setIframeError(true)}
              />
      ...
      <Dialog open={showLibraryPrompt} onClose={() => setShowLibraryPrompt(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ textAlign: 'center', pt: 4 }}>
          🎉 Course Completed!
        </DialogTitle>
        <DialogContent sx={{ textAlign: 'center', pb: 1 }}>
          <Typography variant="body1" sx={{ mb: 2 }}>
            Would you like to add <strong>{course?.title}</strong> to your Library for permanent access?
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Saved courses remain accessible in your Library even after the term ends.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ justifyContent: 'center', gap: 2, pb: 3, px: 3 }}>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                await apiService.post('/student-portal/library/add-course', { courseId });
          ...
          >
            {addingToLibrary ? 'Adding...' : 'Yes, Add to Library'}
          </Button>
        </DialogActions>
      </Dialog>
    </Container>
