...
const TopicSearch: React.FC<TopicSearchProps> = ({
  ...
  const searchTopics = useCallback(async (query: string) => {
    ...
    try {
      ...
      setTopics(results);
      // Position dropdown under the input using getBoundingClientRect to escape overflow:hidden parents
      if (inputRef.current) {
  ...
  return (
    <div className="topic-search" ref={searchRef}>
      ...
      <div className="topic-search-input-wrapper">
        <div className="topic-search-input-container">
          <input
            ...
            onFocus={() => {
              if (searchQuery.length >= 2 && topics.length > 0) {
                if (inputRef.current) {
                  const rect = inputRef.current.getBoundingClientRect();
                  setDropdownStyle({
                    position: 'fixed',
        ...
        {showDropdown && topics.length > 0 && createPortal(
          <div className="topic-search-dropdown" style={dropdownStyle}>
            {topics.map((topic) => (
              <div
                key={topic.id}
                className={`topic-search-item ${selectedTopic?.id === topic.id ? 'selected' : ''}`}
                onMouseDown={(e) => { e.preventDefault(); handleTopicSelect(topic); }}
              >
        ...
        {showDropdown && searchQuery.length >= 2 && topics.length === 0 && !loading && createPortal(
          <div className="topic-search-dropdown" style={dropdownStyle}>
      ...
      {required && !selectedTopic && !showDropdown && (
