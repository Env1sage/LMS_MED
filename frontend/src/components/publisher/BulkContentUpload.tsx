...
import { BookOpen, Video, Upload, X, Check, FileText, Plus, Trash2, Download, Search, ChevronDown, Image, FileUp } from 'lucide-react';
...
const BulkContentUpload: React.FC<BulkContentUploadProps> = ({ onSuccess }) => {
  ...
  const handleCoverSelect = (id: string, file: File | null) => {
    ...
    updateRow(id, 'coverImage', file);
    // Generate preview
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setCoverPreviews(prev => ({ ...prev, [id]: reader.result as string }));
      };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const coverImages = rows.map(r => r.coverImage);
      const metadata = rows.map((r, i) => ({
        type: r.type,
        title: r.title,
        ...
        hasCoverImage: !!r.coverImage,
        coverImageIndex: r.coverImage ? i : -1,
      }));

      const data = await learningUnitService.bulkUploadFiles(files, metadata, coverImages);
      setResult(data);
...
"VIDEO","Cardiac Examination Technique","Step by step cardiac exam","Medicine","Cardiovascular System","KH","15","cardiology,clinical-skills"`;
  ...
  const handleCsvImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const compIdx = headers.findIndex(h => h.includes('competenc'));
        ...
        const validDiffLevels = diffLevels.map(d => d.value);
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const type: 'BOOK' | 'VIDEO' = rawType === 'VIDEO' ? 'VIDEO' : 'BOOK';
          ...
          const competencyCodes = compIdx >= 0 ? cols[compIdx] || '' : '';
        ...
        if (newRows.length === 0) {
          setError('No valid rows found in CSV. Ensure each row has at least a title.');
          return;
        }

        // Replace empty initial rows, or append to existing rows that have data
        const hasOnlyEmpty = rows.length === 1 && !rows[0].title && !rows[0].file;
        setRows(hasOnlyEmpty ? newRows : [...rows, ...newRows]);
        setError('');
        setResult(null);
      } catch {
  ...
  return (
    <div style={{ padding: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{ display: 'none' }}
          />
          <button onClick={() => csvInputRef.current?.click()} className="bo-btn bo-btn-outline" style={{ fontSize: 12, background: 'rgba(99,102,241,0.08)' }}>
      ...
      {/* CSV import hint: show when rows have no files */}
      {rows.length > 0 && rows.some(r => !r.file && r.title) && (
        <div style={{ padding: '10px 14px', background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 8, color: '#1D4ED8', fontSize: 13, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
          <FileUp size={14} /> Some rows still need files attached. Use the file picker in each row to upload the actual content (PDF/EPUB/MP4).
        </div>
      )}
            ...
            <div>
              ...
              {row.coverImage ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {coverPreviews[row.id] && (
                    <img src={coverPreviews[row.id]} alt="Cover" style={{ width: 36, height: 48, objectFit: 'cover', borderRadius: 4, border: '1px solid #E5E7EB' }} />
                  )}
                  <div style={{ flex: 1, padding: '8px 10px', borderRadius: 6, border: '1px solid #10B981', background: '#F0FDF4', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
                    <span style={{ color: '#166534', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 120 }}>
                      ✅ {row.coverImage.name}
                    </span>
                    <button type="button" onClick={() => handleCoverSelect(row.id, null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#DC2626', fontSize: 14, padding: 0 }}>×</button>
                  </div>
                </div>
              ) : (
                <div>
                  <input type="file" ref={el => { coverRefs.current[row.id] = el; }}
                    accept=".jpg,.jpeg,.png,.webp"
                    onChange={e => handleCoverSelect(row.id, e.target.files?.[0] || null)}
                    style={{ display: 'none' }} />
                  <button type="button" onClick={() => coverRefs.current[row.id]?.click()}
                    style={{
                      ...inputStyle, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6,
                      color: 'var(--bo-text-muted)', border: '1px dashed var(--bo-border)',
                    }}>
                    <Image size={14} /> Choose cover...
                  </button>
                </div>
              )}
            </div>
