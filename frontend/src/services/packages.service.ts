...
export const packagesService = {
  ...
  /**
   * Get all learning units in a specific package
   */
  getPackageContent: async (packageId: string): Promise<{
    package: { id: string; name: string; subjects: string[]; contentTypes: string[] };
    total: number;
    learningUnits: Array<{
      id: string;
      title: string;
    ...
    const response = await apiService.get(`/packages/${packageId}/content`);
    return response.data;
  },
};
