...
class BitflowOwnerService {
  ...
  async getAllContent(params: {
    ...
    const response = await apiService.get('/bitflow-owner/content', { params });
    const raw = response.data;
    // Normalize backend {content, total, page, limit, totalPages} → {data, meta}
    return {
      data: raw.content || raw.data || [],
      meta: {
        total: raw.total ?? raw.meta?.total ?? 0,
        page: raw.page ?? raw.meta?.page ?? 1,
        limit: raw.limit ?? raw.meta?.limit ?? 50,
