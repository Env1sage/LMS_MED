...
const mcqService = {
  ...
  bulkVerify: async (ids?: string[]): Promise<{ verified: number; alreadyVerified: number }> => {
    const response = await axios.post(`${API_BASE_URL}/bulk-verify`,
  ...
  bulkUnverify: async (ids?: string[]): Promise<{ unverified: number }> => {
    const response = await axios.post(`${API_BASE_URL}/bulk-unverify`,
      ids && ids.length > 0 ? { ids } : {},
      { headers: getAuthHeaders() }
    );
    return response.data;
