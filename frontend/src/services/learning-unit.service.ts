...
const learningUnitService = {
  ...
  bulkUploadFiles: async (files: File[], metadata: any[], coverImages?: (File | null)[]): Promise<{
    success: number;
    failed: number;
    errors: string[];
    ...
    if (coverImages) {
      coverImages.forEach((img, idx) => {
        if (img) formData.append('coverImages', img, `cover_${idx}_${img.name}`);
      });
    }
  ...
  activateContent: async (id: string): Promise<LearningUnit> => {
    const response = await apiService.patch<LearningUnit>(`/learning-units/${id}/status`, { status: 'ACTIVE' });
    return response.data;
  },
