...
const governanceService = {
  ...
  bulkUploadFaculty: async (file: File): Promise<{
    ...
    if (!response.ok) {
      let errorData: any = { message: `Upload failed (${response.status})` };
      try { errorData = await response.json(); } catch { /* non-JSON error body */ }
      const err = new Error(errorData.message || 'Failed to upload faculty users');
  ...
  // Reset faculty credentials — generates new password, updates DB, emails faculty
  resetFacultyCredentials: async (id: string): Promise<{ message: string; newPassword: string }> => {
    const response = await apiService.post<{ message: string; newPassword: string }>(
      `/governance/faculty-users/${id}/reset-credentials`
    );
    return response.data;
  },
  ...
  getHodStudents: async (params?: {
    ...
    const response = await apiService.get('/governance/faculty-users/hod-students', { params });
    return response.data;
  },
  ...
  getNotifications: async (query?: { type?: string; priority?: string; isActive?: boolean }): Promise<Notification[]> => {
    const response = await apiService.get<{ data: Notification[]; meta: any } | Notification[]>('/governance/notifications', { params: query });
