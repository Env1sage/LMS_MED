...
export class PackagesController {
  ...
  @Roles(UserRole.BITFLOW_OWNER, UserRole.COLLEGE_ADMIN, UserRole.COLLEGE_DEAN)
  async getCollegePackages(@Param('collegeId') collegeId: string, @Request() req: any) {
    if ((req.user.role === UserRole.COLLEGE_ADMIN || req.user.role === UserRole.COLLEGE_DEAN) && req.user.collegeId !== collegeId) {
      throw new ForbiddenException('You can only access packages for your own college');
    }
  ...
  @Roles(UserRole.BITFLOW_OWNER, UserRole.COLLEGE_ADMIN, UserRole.COLLEGE_DEAN, UserRole.FACULTY, UserRole.COLLEGE_HOD)
  async getCollegeAvailableContent(@Param('collegeId') collegeId: string, @Request() req: any) {
    if ([UserRole.FACULTY, UserRole.COLLEGE_HOD, UserRole.COLLEGE_ADMIN, UserRole.COLLEGE_DEAN].includes(req.user.role)) {
      if (req.user.collegeId !== collegeId) {
        throw new ForbiddenException('You can only access content for your own college');
      }
    }
  ...
  /**
   * Get all content (learning units) in a specific package
   */
  @Get(':id/content')
  @Roles(UserRole.BITFLOW_OWNER, UserRole.PUBLISHER_ADMIN, UserRole.COLLEGE_ADMIN, UserRole.COLLEGE_DEAN)
  async getPackageContent(@Param('id') id: string, @Request() req: any) {
    if (req.user.role === UserRole.COLLEGE_ADMIN || req.user.role === UserRole.COLLEGE_DEAN) {
      await this.packagesService.assertPackageAssignedToCollege(id, req.user.collegeId);
    }
    return this.packagesService.getPackageContent(id);
  }
