...
export class FacultyAssignmentMgmtService {
  ...
  async createAssignment(facultyId: string, collegeId: string, dto: any) {
    ...
    const assignment = await this.prisma.tests.create({
      data: {
        ...
        totalQuestions: dto.mcqIds?.length || dto.totalQuestions || 0,
        totalMarks: dto.totalMarks || (dto.mcqIds?.length || 0) * (dto.marksPerQuestion || 1),
    ...
    // ── Add MCQ questions to assignment ──
    if (dto.mcqIds && Array.isArray(dto.mcqIds) && dto.mcqIds.length > 0) {
      const marksPerQ = dto.marksPerQuestion || 1;
      const questionData = dto.mcqIds.map((mcqId: string, index: number) => ({
        id: uuidv4(),
      ...
      await this.prisma.test_questions.createMany({ data: questionData });

      // Update total questions and marks
      await this.prisma.tests.update({
        where: { id: assignment.id },
        data: {
          totalQuestions: dto.mcqIds.length,
          totalMarks: dto.mcqIds.length * marksPerQ,
        },
      });
    }
  ...
  /** Get available MCQs for assignment creation (publisher content + own MCQs) */
  async getAvailableMcqs(
    facultyId: string,
    collegeId: string,
    filters: { subject?: string; topic?: string; search?: string },
  ) {
    // Get publisher IDs from packages assigned to the college
    const assignedPackages = await this.prisma.college_packages.findMany({
      where: { collegeId, status: 'ACTIVE' },
      include: { package: { select: { publisherId: true } } },
    });
    const assignedPublisherIds = [...new Set(assignedPackages.map(cp => cp.package.publisherId))];

    // Build where: MCQs from assigned publishers OR created by this faculty
    const publisherOrOwn: any[] = [
      { createdBy: facultyId },
    ];
    if (assignedPublisherIds.length > 0) {
      publisherOrOwn.push({ publisherId: { in: assignedPublisherIds } });
    }
    ...
    const mcqs = await this.prisma.mcqs.findMany({
      where,
      select: {
        id: true, question: true, questionImage: true,
        optionA: true, optionB: true, optionC: true, optionD: true,
        correctAnswer: true, explanation: true,
        subject: true, topic: true, difficultyLevel: true,
        mcqType: true, publisherId: true, createdBy: true,
      },
      orderBy: { createdAt: 'desc' },
      take: 500,
    });
    ...
    return mcqs.map(m => ({
      ...m,
      source: m.publisherId ? 'PUBLISHER' : 'FACULTY',
    }));
  }
}
