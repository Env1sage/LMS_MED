...
export class TopicsService {
  ...
  async create(dto: CreateTopicDto, userId: string) {
    ...
    if (existingTopic && existingTopic.status !== ContentStatus.INACTIVE) {
      throw new ConflictException(`Topic with code ${dto.code} already exists`);
    }

    let topic;
    if (existingTopic && existingTopic.status === ContentStatus.INACTIVE) {
      // Reactivate the previously deactivated topic with new data
      topic = await this.prisma.topics.update({
      ...
      topic = await this.prisma.topics.create({
        data: {
          subject: dto.subject,
          name: dto.name,
          code: dto.code,
