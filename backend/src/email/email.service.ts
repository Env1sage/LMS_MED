...
export class EmailService {
  ...
  private getCredentialEmailTemplate(data: CredentialEmailData): string {
    ...
    const loginUrl = data.loginUrl || `${this.configService.get<string>('FRONTEND_URL') || 'http://52.66.165.194'}/login`;
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${this.configService.get<string>('FRONTEND_URL') || 'http://52.66.165.194'}/student" style="display: inline-block; background: linear-gradient(135deg, #c47335 0%, #a85d2a 100%); color: white; padding: 14px 40px; text-decoration: none; border-radius: 8px; font-weight: 600;">View Course</a>
    </div>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${this.configService.get<string>('FRONTEND_URL') || 'http://52.66.165.194'}/student" style="display: inline-block; background: linear-gradient(135deg, #c47335 0%, #a85d2a 100%); color: white; padding: 14px 40px; text-decoration: none; border-radius: 8px; font-weight: 600;">View Detailed Results</a>
    </div>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
    ...
    <div style="text-align: center; margin: 30px 0;">
      <a href="${this.configService.get<string>('FRONTEND_URL') || 'http://52.66.165.194'}" style="display: inline-block; background: linear-gradient(135deg, #c47335 0%, #a85d2a 100%); color: white; padding: 14px 40px; text-decoration: none; border-radius: 8px; font-weight: 600;">View in Portal</a>
    </div>
  </div>
</body>
</html>
  ...
  /**
   * Send a college notification email to multiple recipients in bulk
   */
  async sendBulkNotificationEmails(data: {
    ...
    if (!this.isConfigured) {
      this.logger.warn(`Email not configured. Would send notification "${data.title}" to ${data.recipients.length} recipients`);
      return;
    }
    ...
    const borderColor = priorityColors[data.priority] || '#2563EB';
    const from = this.configService.get<string>('SMTP_FROM') || '"Bitflow Medical LMS" <noreply@bitflow.com>';
    const subjectPrefix = data.priority === 'URGENT' ? '🚨 URGENT' : data.priority === 'HIGH' ? '⚠️ Important' : '📢';
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto;padding:20px;background:#f5f5f5;">
  <div style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);padding:30px;border-radius:12px 12px 0 0;text-align:center;">
    <h1 style="color:#c47335;margin:0;">Bitflow Medical LMS</h1>
    ${data.collegeName ? `<p style="color:#a0a0a0;margin:8px 0 0;font-size:14px;">${data.collegeName}</p>` : ''}
  </div>
  <div style="background:#fff;padding:30px;border-radius:0 0 12px 12px;box-shadow:0 4px 6px rgba(0,0,0,0.1);">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;">
      <span style="display:inline-block;padding:3px 12px;border-radius:20px;font-size:12px;font-weight:700;background:${borderColor}20;color:${borderColor};">${data.priority}</span>
      <span style="font-size:12px;color:#6B7280;">To: ${data.audience}</span>
    </div>
    <h2 style="color:#1a1a2e;margin:0 0 8px;">Hi ${recipientName},</h2>
    <p style="color:#6B7280;font-size:14px;margin:0 0 20px;">A new notification from ${data.senderName}:</p>
    <div style="background:#f8f9fa;border-left:4px solid ${borderColor};padding:20px;margin:0 0 25px;border-radius:0 8px 8px 0;">
      <h3 style="margin:0 0 12px;color:#1a1a2e;font-size:16px;">${data.title}</h3>
      <p style="margin:0;white-space:pre-wrap;font-size:14px;">${data.message}</p>
    </div>
    <div style="text-align:center;">
      <a href="${this.configService.get<string>('FRONTEND_URL') || 'http://52.66.165.194'}" style="display:inline-block;background:linear-gradient(135deg,#c47335,#a85d2a);color:#fff;padding:12px 36px;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px;">View in Portal</a>
    </div>
  </div>
  <div style="text-align:center;padding:16px;color:#999;font-size:12px;">
    <p style="margin:4px 0;">© ${new Date().getFullYear()} Bitflow Medical LMS. All rights reserved.</p>
  </div>
</body>
</html>`;
    ...
    for (let i = 0; i < data.recipients.length; i += batchSize) {
      ...
      await Promise.allSettled(
        batch.map(r =>
          this.transporter.sendMail({ from, to: r.email, subject, html: buildHtml(r.fullName) })
            .then(() => this.logger.debug(`Notification email sent to ${r.email}`))
            .catch(err => this.logger.error(`Notification email failed for ${r.email}: ${err.message}`))
        )
      );
    ...
    this.logger.log(`Bulk notification emails sent: "${data.title}" → ${data.recipients.length} recipients`);
  }
}
