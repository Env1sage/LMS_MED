...
export class CourseAnalyticsService {
  ...
  async getTeacherContent(collegeId?: string) {
    const whereClause: any = {};
    ...
    for (const course of courses) {
      const fId = course.users.id;
      if (!byFaculty[fId]) {
        byFaculty[fId] = {
          faculty: course.users,
          courses: [],
          totalCourses: 0,
          totalContent: 0,
        };
      }
      byFaculty[fId].courses.push(course);
      byFaculty[fId].totalCourses++;
      byFaculty[fId].totalContent += course.learning_flow_steps.length;
    }

    return {
      totalCourses: courses.length,
      totalFaculty: Object.keys(byFaculty).length,
      facultyContent: Object.values(byFaculty),
    };
  }
}
